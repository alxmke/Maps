...
def find_closest(location, centroids):
    ...
    """
    # BEGIN Question 3
    return min([[distance(location, centroid), centroid] for centroid in centroids], key = lambda x: x[0])[1]
    # END Question 3
...
def find_predictor(user, restaurants, feature_fn):
    ...
    xy_zipped = zip(xs, ys)
    S_xy = sum([(x - mean_xs)*(y - mean_ys) for x, y in xy_zipped])
...
def best_predictor(user, restaurants, feature_fns):
    ...
    r_squared_predictor_fn_pairs = []

    for function in feature_fns:
        predictor_fn, r_squared = find_predictor(user, reviewed, function)
        r_squared_predictor_fn_pairs.append([r_squared, predictor_fn])

    best_r_squared_predictor_fn = max(r_squared_predictor_fn_pairs, key=lambda l: l[0])[1]

    return best_r_squared_predictor_fn
    # END Question 8
